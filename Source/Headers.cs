...
namespace PowerUI.Http{
	...
	public class Headers{
		...
		/// <summary>Applies this header set from a status line and a web header set.</summary>
		public void Apply(string statusLine, System.Net.WebHeaderCollection webHeaders, int contentLength){
			lines = ParseHeaders(statusLine, webHeaders);
			
			if(contentLength >= 0){ // -1 means not present
				AddIfNew("content-length",contentLength.ToString());
			}
		}
		
		/// <summary>Converts a raw header string to a set of headers.
		/// The status line is indexed as the empty string.</summary>
		public static Dictionary<string,List<string>> ParseHeaders(string statusLine, System.Net.WebHeaderCollection webHeaders){
			...
			for(int i = 0; i < webHeaders.Count; i++)
			{
				string headerType = webHeaders.GetKey(i);
				set=new List<string>();
				headers[headerType.ToLower()]=set;
				foreach(string value in webHeaders.GetValues(i))
				{
					set.Add(value);
				}
			}
